...
> [!NOTE]
> Hệ thống **FPTU PE Grading System** đã nâng cấp hoàn chỉnh mô hình **Microservices chuẩn doanh nghiệp** bao gồm 2 REST API Services độc lập, 1 gRPC Calculation Service, 1 Background Worker & Event Consumer, cùng 2 CSDL riêng biệt (**Database-per-Service**: `FptuUserDb` và `FptuGradingDb`).

###  Tóm tắt mức độ đáp ứng:
- **Tình trạng biên dịch:** Solution C# biên dịch thành công 100% (`0 Errors`).
- **User & Identity REST API:**  Đạt 100% (Microservice độc lập [FptuGradingSystem.UserAPI](file:///d:/PRN_AS/be/FptuGradingSystem.UserAPI) trên Port 5002, kết nối `FptuUserDb`).
- **Grading Core REST API:**  Đạt 100% (Microservice độc lập [FptuGradingSystem.API](file:///d:/PRN_AS/be/FptuGradingSystem.API) trên Port 5000, kết nối `FptuGradingDb`).
...
- **Docker Deployment:**  Đạt 100% ([docker-compose.yml](file:///d:/PRN_AS/be/docker-compose.yml) định nghĩa 6 container: UserAPI, GradingAPI, gRPC, Worker, Postgres, Redis).
...
|---|---|---|---|
| **User & Auth API Service** (Item 2.1) | ** Đã hoàn thành** | - ASP.NET Core REST API độc lập [FptuGradingSystem.UserAPI](file:///d:/PRN_AS/be/FptuGradingSystem.UserAPI) (Port 5002).<br>- Đăng ký, Đăng nhập, Cấp JWT Token và Quản lý Danh sách Giảng viên.<br>- CSDL riêng `FptuUserDb`. | **100%** |
| **Grading Core REST API** (Item 2.1) | ** Đã hoàn thành** | - ASP.NET Core REST API [FptuGradingSystem.API](file:///d:/PRN_AS/be/FptuGradingSystem.API) (Port 5000).<br>- Endpoints CRUD cho `Subjects`, `Rubrics`, `ExamClasses`, `Submissions`, `Grades`.<br>- Tìm kiếm, lọc và phân trang tại [SubmissionsController.cs](file:///d:/PRN_AS/be/FptuGradingSystem.API/Controllers/SubmissionsController.cs).<br>- CSDL riêng `FptuGradingDb`. | **100%** |
...
Sơ đồ mô hình triển khai Docker Compose (6 Containers):

```mermaid
graph TD
    subgraph "Docker Bridge Network (grading-network)"
        c_user["👤 grading-user-api (Port 5002:8080)"]
    ...
    c_user -->|Database FptuUserDb| c_postgres
...
|---|---|---|---|
| **System Architecture & Design** | 20% | **20 / 20** | Kiến trúc Microservices + Database-per-Service + Clean Architecture tuyệt hảo. |
| **REST API Implementation** | 20% | **20 / 20** | 2 REST API Services độc lập (UserAPI & GradingAPI), Clean Code, CQRS. |
...
| **Docker / Cloud Deployment** | 10% | **10 / 10** | Dockerfile và docker-compose.yml 6 containers hoàn hảo. |
